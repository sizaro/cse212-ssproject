...
public static class Recursion
{
    ...
    {
        if (n <= 0)
            return 0; // Base case: if n is 0 or less, return 0
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        if (word.Length == size)
        {
            results.Add(word); // Base case: add the permutation to results
            return;
        }

        for (int i = 0; i < letters.Length; i++)
        {
            PermutationsChoose(results, letters.Remove(i, 1), size, word + letters[i]); // Recursive case
        }
    }
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        remember[s] = ways; // Store in dictionary
        return ways;
    }
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        if (index == -1)
        {
            results.Add(pattern); // Base case: no wildcard left, add to results
            return;
        }
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // Base case: If we reach the end, add the path to results
        if (maze.IsEnd(x, y))
        {
            currPath.Add((x, y)); // Add the final position to the path
            results.Add(currPath.AsString()); // Use the AsString extension method to format the path
        ...
        foreach (var direction in new (int, int)[] { (0, 1), (1, 0), (0, -1), (-1, 0) })
        {
            int newX = x + direction.Item1, newY = y + direction.Item2;
